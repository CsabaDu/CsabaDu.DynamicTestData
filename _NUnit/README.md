...
- [**How it Works**](#how-it-works)
  ...
  - [Abstract DynamicDataSourceBase Class](#abstract-dynamicdatasource-class)
...
**`DynamicDataSourceBase` Abstract Class** (Updated v1.1.0):
- The `DynamicDataSourceBase` class provides methods (`TestDataToArgs`, `TestDataReturnsToArgs`, `TestDataThrowsToArgs`) to convert test data into arguments for test methods.
...
2. **Create a derived dynamic test data source class**:
  - Create one class for each test class separately that extends the `DynamicDataSourceBase` base class.
...
**`DynamicDataSourceBase` Abstract Class**
(Updated v1.1.0)
...
### **Abstract `DynamicDataSourceBase` Class**
(Updated v1.2.3)
...
However `DynamicDataSourceBase` class implements all necessary methods for test data preparation, it is marked as `abstract`. Intended usage is to
...
`ArgsCode ArgsCode` is the only property of `DynamicDataSourceBase` class. This property is marked as `protected`. It should be initalized with the constructor parameter of the class. This property will be the parameter of the `ToArgs` methods called by the object array generator methods of the class
...
`DynamicDataSourceBase` class provides a dedicated object array generator each `TestData type`. The methods' parameters types and sequences are the same as the constructors' parameters of the related `TestData` types.
...
public class NativeTestDataSource(ArgsCode argsCode) : DynamicDataSourceBase(argsCode)
{
...
public class TestCaseDataSource(ArgsCode argsCode) : DynamicDataSourceBase(argsCode)
{
    private readonly DateTime DateTimeNow = DateTime.Now;
...
- Includes the `ITestData` generic interface types, `TestData` record types, `DynamicDataSourceBase` base class, and `ArgsCode` enum.
...
- **Added**:
  - `OptionalToArgs` method added to the `DynamicDataSourceBase` class.
  - `DisposableMemento` private class added to the `DynamicDataSourceBase` class.
  - `ArgsCode` property behavior of the `DynamicDataSourceBase` class changed.
- **Note**: This update is backward-compatible with previous versions.

#### **Version 1.1.1** (2025-03-27)
- **Changed**: `private DynamicDataSourceBase._tempArgsCode` to `protected DynamicDataSourceBase.tempArgsCode`, to allow for easier extension of the DynamicDataSourceBase class.
- **Updated**: README.md and fixed navigation anchors.

### **Version 1.2.0** (2025-03-28)

- **Added**: protected static generic `WithOptionalArgsCode<>` methods to the `DynamicDataSourceBase` class to support the extension of using the optional `ArgsCode?` parameter in the derived data source classes. 
- **Changed**:
  - `OptionalToArgs` method to call a `WithOptionalArgsCode<>` method.
  - `protected DynamicDataSourceBase.tempArgsCode` back to `private DynamicDataSourceBase._tempArgsCode`.
...
- **Updated**: README.md Abstract`DynamicDataSourceBase` Class section corrected.

### **Version 1.3.0** (2025-05-06)

- **Added**: `ITestDataReturns` and `ITestDataThrows` base marker interfaces. 
- **Updated**: README.md updated and Abstract`DynamicDataSourceBase` Class section corrected.
- **Note**: This update is backward-compatible with previous versions.
